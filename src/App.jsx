...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-hover:hover { transform: translateY(-4px) scale(1.02); box-shadow: 0 16px 40px rgba(0,0,0,0.22) !important; }
  .btn-hover:hover { filter: brightness(1.10); transform: translateY(-1px); }
...
const T = {
  white:"#FFFFFF", navy:"#1A365D", slate:"#2A4365", coral:"#FF6B6B",
  off:"#F7FAFC", navyMid:"#2C5282", grey:"#718096", greyL:"#EDF2F7", greyM:"#CBD5E0",
...
// ── Vibrant component colours ─────────────────────────────────
const COMPONENTS = [
  {id:1, name:"Sports for Development",                        icon:"⚽", color:"#E53935", light:"#FFEBEE"},
  {id:2, name:"Youth Leadership & Mentorship",                 icon:"🌟", color:"#1E88E5", light:"#E3F2FD"},
  {id:3, name:"Entrepreneurship & Skills Development",         icon:"💼", color:"#FB8C00", light:"#FFF3E0"},
  {id:4, name:"Education Support",                             icon:"🎓", color:"#43A047", light:"#E8F5E9"},
  {id:5, name:"Health & Wellness",                             icon:"❤️", color:"#E91E63", light:"#FCE4EC"},
  {id:6, name:"Disability Inclusion & Social Protection",      icon:"♿", color:"#8E24AA", light:"#F3E5F5"},
  {id:7, name:"Environmental Sanitation & Community Service",  icon:"🌿", color:"#00897B", light:"#E0F2F1"},
];

// ── Summary card colours (vivid) ─────────────────────────────
const STAT_COLORS = [
  {label:"Total Beneficiaries", icon:"👥", bg:"#E53935", text:"#FFFFFF"},
  {label:"Active",              icon:"✅", bg:"#43A047", text:"#FFFFFF"},
  {label:"Completed",           icon:"🏁", bg:"#1E88E5", text:"#FFFFFF"},
  {label:"Male",                icon:"♂",  bg:"#8E24AA", text:"#FFFFFF"},
  {label:"Female",              icon:"♀",  bg:"#FB8C00", text:"#FFFFFF"},
];

const AC=["#E53935","#1E88E5","#FB8C00","#43A047","#E91E63","#8E24AA","#00897B"];
...
function sPill(s){if(s==="Active")return{bg:"#C8E6C9",color:"#1B5E20"};if(s==="Completed")return{bg:"#BBDEFB",color:"#0D47A1"};return{bg:"#FFF9C4",color:"#F57F17"};}
...
const Btn=({children,onClick,variant="primary",style:sx})=>{
  ...
  const v=variant==="primary"?{background:"#E53935",color:"#fff"}:variant==="navy"?{background:T.navy,color:"#fff"}:variant==="danger"?{background:"#C62828",color:"#fff"}:{background:T.off,color:T.navy,border:`1px solid ${T.greyM}`};
...
const FI=({label,value,onChange,type="text",options,full,readOnly,placeholder})=>(
  <div style={{display:"flex",flexDirection:"column",gap:5,...(full?{gridColumn:"1/-1"}:{})}}>
    ...
    {options
      ...
      :type==="textarea"
        ?<textarea value={value||""} onChange={e=>onChange&&onChange(e.target.value)} readOnly={readOnly} placeholder={placeholder||""} style={{border:`1px solid ${T.greyM}`,borderRadius:8,padding:"9px 14px",fontSize:13,fontFamily:"'Source Sans 3',sans-serif",background:readOnly?T.off:"#fff",color:T.navy,minHeight:80,resize:"vertical"}}/>
...
function Login({onLogin,users,logoUrl}){
  ...
  return(<div style={{minHeight:"100vh",background:`linear-gradient(135deg,${T.navy} 0%,#2C5282 50%,#1A365D 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{background:"#fff",borderRadius:20,padding:"44px 40px",width:400,boxShadow:"0 24px 64px rgba(0,0,0,0.35)"}}>
      <div style={{textAlign:"center",marginBottom:28}}><Logo size={56} color={T.navy} url={logoUrl}/><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:700,color:T.navy,marginTop:10}}>OGB App</div><div style={{fontSize:12,color:T.grey,marginTop:4}}>LYSBF · Community Youth Empowerment Programme</div></div>
      {err&&<div style={{background:"#FFEBEE",color:"#C62828",borderRadius:8,padding:"10px 14px",fontSize:13,marginBottom:14,textAlign:"center"}}>{err}</div>}
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <button onClick={go} style={{background:"#E53935",color:"#fff",border:"none",borderRadius:10,padding:"13px",fontSize:15,fontWeight:700,fontFamily:"'Source Sans 3',sans-serif",cursor:"pointer"}}>Sign In →</button>
      </div>
...
function Sidebar({user,page,setPage,onLogout,logoUrl}){
  ...
  return(<div style={{width:248,minHeight:"100vh",background:`linear-gradient(180deg,#1A365D 0%,#2A4365 100%)`,display:"flex",flexDirection:"column",flexShrink:0,position:"fixed",top:0,left:0,bottom:0,zIndex:100,overflowY:"auto"}}>
    <div style={{padding:"22px 18px 16px",borderBottom:"1px solid rgba(255,255,255,0.12)"}}><div style={{display:"flex",alignItems:"center",gap:10}}><Logo size={36} color="#fff" url={logoUrl}/><div><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,fontWeight:700,color:"#fff"}}>OGB App</div><div style={{fontSize:9,color:"rgba(255,255,255,0.45)",letterSpacing:2,textTransform:"uppercase"}}>LYSBF · CYEP</div></div></div></div>
    <div style={{padding:"14px 10px",flex:1}}>
      ...
      {user.role==="Admin"&&<><div style={{margin:"14px 0 6px",padding:"0 14px",fontSize:10,color:"rgba(255,255,255,0.35)",letterSpacing:2,textTransform:"uppercase"}}>Admin</div><NI label="User Management" icon="🔐" p="users"/><NI label="Settings" icon="⚙️" p="settings"/></>}
    </div>
    <div style={{padding:"14px 16px",borderTop:"1px solid rgba(255,255,255,0.12)"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:"50%",background:"#E53935",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:13,color:"#fff",flexShrink:0}}>{user.avatar}</div><div style={{flex:1,minWidth:0}}><div style={{color:"#fff",fontSize:12,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.name}</div><div style={{fontSize:10,color:"rgba(255,107,107,0.9)",background:"rgba(255,107,107,0.18)",padding:"2px 8px",borderRadius:20,display:"inline-block",marginTop:2}}>{user.role}</div></div><span onClick={onLogout} style={{color:"rgba(255,255,255,0.35)",cursor:"pointer",fontSize:18}}>⇠</span></div></div>
  </div>);
}
  ...
  return(<div className="fade-in"><Topbar title="Dashboard" sub="View current tasks, activities and reports"/>
    <div style={{padding:"28px 32px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:14,marginBottom:36}}>
        {STAT_COLORS.map((s,i)=>(<div key={s.label} style={{background:s.bg,borderRadius:14,padding:"20px 16px",textAlign:"center",boxShadow:"0 4px 14px rgba(0,0,0,0.18)",transition:"transform 0.2s"}} className="card-hover">
          <div style={{fontSize:26,marginBottom:8}}>{s.icon}</div>
          <div style={{fontSize:34,fontWeight:700,color:s.text,fontFamily:"'Playfair Display',serif",lineHeight:1}}>{counts[i]}</div>
          <div style={{fontSize:11,color:"rgba(255,255,255,0.85)",marginTop:6,textTransform:"uppercase",letterSpacing:1,fontWeight:600}}>{s.label}</div>
        </div>))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
        {COMPONENTS.map(c=>{const count=vis.filter(b=>b.component_id===c.id).length;return(
          <div key={c.id} className="card-hover" style={{borderRadius:14,overflow:"hidden",boxShadow:"0 4px 14px rgba(0,0,0,0.14)",transition:"all 0.22s",cursor:"pointer"}}>
            <div style={{background:c.color,padding:"18px 16px 14px"}}>
              <div style={{fontSize:32,marginBottom:6}}>{c.icon}</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.88)",fontWeight:600,lineHeight:1.3}}>{c.name}</div>
            </div>
            <div style={{background:"#fff",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{fontSize:24,fontWeight:700,color:c.color,fontFamily:"'Playfair Display',serif"}}>{count}</div>
            </div>
          </div>);})}
      </div>
    </div>
...
function BenList({bens,user,users,onView,onEdit,onSIR}){
  ...
  const vis=(user.role==="Admin"?bens:bens.filter(b=>b.assigned_to===user.id)).filter(b=>(!search||b.name?.toLowerCase().includes(search.toLowerCase())||b.bid?.includes(search))&&(compF==="all"||b.component_id===Number(compF))&&(commF==="all"||b.community===commF)&&(statF==="all"||b.status===statF)).sort((a,b2)=>{if(sort==="name-asc")return(a.name||"").localeCompare(b2.name||"");if(sort==="name-desc")return(b2.name||"").localeCompare(a.name||"");if(sort==="updated-desc")return(b2.last_follow_up||"").localeCompare(a.last_follow_up||"");return 0;});
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>
            {vis.map((b,i)=>{const c=comp(b.component_id);return(<tr key={b.id} className="row-hover" style={{background:i%2===0?"#fff":T.off,borderBottom:`1px solid ${T.greyL}`,transition:"background 0.15s"}}>
              ...
              <td style={{padding:"13px 16px",fontSize:12,color:b.last_follow_up?"#C62828":T.grey,fontWeight:b.last_follow_up?600:400}}>{b.last_follow_up||"Not yet"}</td>
              <td style={{padding:"13px 16px"}}><Pill s={b.status}/></td>
              <td style={{padding:"13px 16px"}}><div style={{display:"flex",gap:5}}>
                <button className="action-btn" onClick={()=>onView(b)} style={{padding:"5px 11px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:"#E3F2FD",color:"#1565C0"}}>👁 View</button>
                {(user.role==="Admin"||b.assigned_to===user.id)&&<button className="action-btn" onClick={()=>onEdit(b)} style={{padding:"5px 11px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:"#E8F5E9",color:"#2E7D32"}}>✏️ Edit</button>}
                <button className="action-btn" onClick={()=>onSIR(b)} style={{padding:"5px 11px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:"#FFF9C4",color:"#F57F17"}}>📋 SIR</button>
              </div></td>
            </tr>);})}
    ...
    <div style={{padding:"24px 32px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:20}}>
        <div style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 2px 10px rgba(0,0,0,0.08)"}}>
          <div style={{background:comp?.color||T.navy,padding:"28px 20px",textAlign:"center"}}>
            <div style={{width:70,height:70,borderRadius:"50%",background:"rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:28,color:"#fff",margin:"0 auto 12px"}}>{inits(ben.name)}</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:19,fontWeight:700,color:"#fff"}}>{ben.name}</div>
            <div style={{fontSize:12,color:"rgba(255,255,255,0.8)",marginTop:3}}>{comp?.icon} {comp?.name}</div>
        ...
        <div>
          ...
          <div style={{background:"#fff",borderRadius:12,padding:"24px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
            ...
            {tab==="followups"&&<div>
              ...
              {(ben.posts||[]).slice().reverse().map(p=>(<div key={p.id} style={{background:T.off,borderRadius:10,padding:"14px 16px",marginBottom:12,borderLeft:"3px solid #E53935"}}><div style={{fontSize:11,color:T.grey,marginBottom:6}}>📅 {p.date} · ✍️ {p.author}</div><div style={{fontSize:13,color:T.navy,lineHeight:1.6}}>{p.text}</div></div>))}
            </div>}
    ...
    <div style={{padding:"24px 32px"}}>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:"36px 40px",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",maxWidth:800,margin:"0 auto"}}>
        <div style={{textAlign:"center",borderBottom:`2px solid ${T.navy}`,paddingBottom:20,marginBottom:24}}>
          ...
          <div style={{fontSize:16,fontWeight:700,color:"#E53935",marginTop:10,textTransform:"uppercase",letterSpacing:2}}>Social Inquiry Report</div>
        ...
        {[{title:"1. Basic Demographic Information",fields:[["Full Name",ben.name],["Date of Birth",ben.dob],["Age",ben.age],["Gender",ben.gender],["Nationality",ben.nationality],["Tribe / Ethnicity",ben.tribe],["Religion",ben.religion],["Region",ben.region],["District",ben.district],["City / Village",ben.city],["Area / Suburb",ben.area],["Physical Location",ben.physical_desc]]},
          ...
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:T.navy,borderLeft:"4px solid #E53935",paddingLeft:12,marginBottom:14}}>{sec.title}</div>
        ...
        <div style={{marginBottom:24}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:T.navy,borderLeft:"4px solid #E53935",paddingLeft:12,marginBottom:14}}>6. Follow-Up Notes</div>
          {(!ben.posts||ben.posts.length===0)?<div style={{color:T.grey,fontSize:13}}>No follow-up notes recorded.</div>:(ben.posts||[]).map((p,i)=>(<div key={i} style={{marginBottom:12,padding:"10px 14px",background:T.off,borderRadius:8,borderLeft:"3px solid #E53935"}}><div style={{fontSize:11,color:T.grey,marginBottom:4}}>Date: {p.date} | Officer: {p.author}</div><div style={{fontSize:13,color:T.navy,lineHeight:1.6}}>{p.text}</div></div>))}
        </div>
  ...
  return(<div className="fade-in"><Topbar title={edit?"Edit Beneficiary":"Register New Beneficiary"} sub="Fill in all required fields"/>
    <div style={{padding:"24px 32px"}}><Btn variant="secondary" onClick={onCancel} style={{marginBottom:20}}>← Cancel</Btn>
      <div style={{background:"#fff",borderRadius:12,padding:"28px 32px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        ...
        <div style={{display:"flex",gap:12}}><Btn variant="primary" style={{opacity:busy?0.6:1}} onClick={async()=>{if(!f.name.trim())return;setBusy(true);await onSave(f);setBusy(false);}}>{busy?"Saving...":edit?"Save Changes":"Register Beneficiary"}</Btn><Btn variant="secondary" onClick={onCancel}>Cancel</Btn></div>
      </div>
    </div>
  </div>);
}
...
function PostsPage({bens,user}){
  ...
  return(<div className="fade-in"><Topbar title="Posts" sub="Your latest follow-up notes and updates"/>
    <div style={{padding:"24px 32px"}}>
      ...
      {all.map((p,i)=>{const c=COMPONENTS.find(x=>x.id===p.comp);return(<div key={i} style={{background:"#fff",borderRadius:12,padding:"18px 20px",marginBottom:14,borderLeft:`4px solid ${c?.color||"#E53935"}`,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div style={{fontWeight:700,color:T.navy,fontSize:14}}>{p.benName} <span style={{color:T.grey,fontWeight:400,fontSize:12}}>({p.bid})</span></div><div style={{fontSize:12,color:T.grey}}>{p.date}</div></div><div style={{fontSize:11,color:T.grey,marginBottom:8}}>{c?.icon} {c?.name}</div><div style={{fontSize:13,color:T.navy,lineHeight:1.65}}>{p.text}</div></div>);})}
    </div>
  </div>);
}
...
function UserMgmt({users,setUsers}){
  ...
  function deleteUser(id){if(!window.confirm("Remove this user?"))return;setUsers(u=>u.filter(x=>x.id!==id));setMsg("✅ User removed.");}
  ...
  return(<div className="fade-in"><Topbar title="User Management" sub="Admin only — manage platform users"/>
    <div style={{padding:"24px 32px"}}>
      ...
      {msg&&<div style={{background:msg.includes("✅")?"#E8F5E9":"#FFEBEE",color:msg.includes("✅")?"#2E7D32":"#C62828",borderRadius:8,padding:"10px 16px",marginBottom:16,fontSize:13}}>{msg}</div>}
      ...
      <div style={{background:"#fff",borderRadius:12,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>{users.map((u,i)=>(<tr key={u.id} style={{background:i%2===0?"#fff":T.off,borderBottom:`1px solid ${T.greyL}`}}>
            ...
            <td style={{padding:"13px 16px"}}><span style={{background:u.role==="Admin"?"#FFEBEE":"#E3F2FD",color:u.role==="Admin"?"#C62828":"#1565C0",padding:"3px 12px",borderRadius:20,fontSize:11,fontWeight:700}}>{u.role}</span></td>
            <td style={{padding:"13px 16px"}}><div style={{display:"flex",gap:6}}>
              <button onClick={()=>{setEditUser({...u});setShowAdd(false);setChangePwUser(null);}} style={{padding:"5px 11px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:"#E3F2FD",color:"#1565C0"}}>✏️ Edit</button>
              <button onClick={()=>{setChangePwUser(u);setShowAdd(false);setEditUser(null);}} style={{padding:"5px 11px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:"#FFF9C4",color:"#F57F17"}}>🔑 Password</button>
              {u.role!=="Admin"&&<button onClick={()=>deleteUser(u.id)} style={{padding:"5px 11px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:"#FFEBEE",color:"#C62828"}}>🗑 Remove</button>}
            </div></td>
          </tr>))}</tbody>
        </table>
      </div>
    ...
    <div style={{padding:"24px 32px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      <div style={{background:"#fff",borderRadius:12,padding:"24px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        ...
        <div style={{border:`2px dashed ${T.greyM}`,borderRadius:12,padding:"28px",textAlign:"center",cursor:"pointer",background:T.off,marginBottom:14}} onClick={()=>fileRef.current.click()}>
          {logoUrl?<img src={logoUrl} alt="logo" style={{width:80,height:80,objectFit:"contain"}}/>:<div><div style={{fontSize:40,marginBottom:8}}>📷</div><div style={{fontSize:13,color:T.grey}}>Click to upload your logo</div><div style={{fontSize:11,color:T.greyM,marginTop:4}}>PNG, JPG or SVG</div></div>}
      ...
      <div style={{background:"#fff",borderRadius:12,padding:"24px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        ...
        <div style={{background:`linear-gradient(135deg,${T.navy},#2C5282)`,borderRadius:12,padding:"24px",textAlign:"center",color:"#fff"}}>
          ...
          <a href="https://lysbfoundation.com/" target="_blank" rel="noreferrer" style={{display:"inline-block",background:"#E53935",color:"#fff",padding:"10px 24px",borderRadius:8,fontSize:13,fontWeight:700,textDecoration:"none"}}>Visit Website ↗</a>
        </div>
      </div>
      <div style={{background:"#fff",borderRadius:12,padding:"24px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
        <SH>Change My Password</SH>
        {msg&&<div style={{background:msg.includes("✅")?"#E8F5E9":"#FFEBEE",color:msg.includes("✅")?"#2E7D32":"#C62828",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:13}}>{msg}</div>}
...
function PostModal({ben,user,onSave,onClose}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
    <div style={{background:"#fff",borderRadius:16,padding:"28px 32px",width:"100%",maxWidth:560,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:"flex",gap:10}}><Btn variant="primary" onClick={async()=>{if(!text.trim())return;setBusy(true);await onSave(ben,text,user);setBusy(false);}}>{busy?"Saving...":"Save Note"}</Btn><Btn variant="secondary" onClick={onClose}>Cancel</Btn></div>
    </div>
  </div>);
}
...
export default function App(){
  ...
  const [viewBen,setView]=useState(null);const [editBen,setEdit]=useState(null);const [sirBen,setSir]=useState(null);const [postModal,setPost]=useState(null);const [logoUrl,setLogoUrl]=useState(null);
  ...
  if(loading)return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:T.off,flexDirection:"column",gap:16}}><div className="spin" style={{width:44,height:44,border:`4px solid ${T.greyM}`,borderTopColor:"#E53935",borderRadius:"50%"}}/><div style={{color:T.grey,fontSize:14}}>Loading OGB App...</div></div>);
