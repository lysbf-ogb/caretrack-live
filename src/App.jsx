...
const FI=({label,value,onChange,type="text",options,full,readOnly,placeholder})=>(
  <div style={{display:"flex",flexDirection:"column",gap:5,...(full?{gridColumn:"1/-1"}:{})}}>
    <Lbl c={label}/>
    {options?<select value={value||""} onChange={e=>onChange&&onChange(e.target.value)} disabled={readOnly} style={{border:`1px solid ${T.greyM}`,borderRadius:8,padding:"9px 14px",fontSize:13,fontFamily:"'Source Sans 3',sans-serif",background:readOnly?T.off:T.white,color:T.navy}}>{options.map(o=><option key={o.value||o} value={o.value||o}>{o.label||o}</option>)}</select>
      :type==="textarea"?<textarea value={value||""} onChange={e=>onChange&&onChange(e.target.value)} readOnly={readOnly} placeholder={placeholder||""} style={{border:`1px solid ${T.greyM}`,borderRadius:8,padding:"9px 14px",fontSize:13,fontFamily:"'Source Sans 3',sans-serif",background:readOnly?T.off:T.white,color:T.navy,minHeight:80,resize:"vertical"}}/>
        :<input type={type} value={value||""} onChange={e=>onChange&&onChange(e.target.value)} readOnly={readOnly} placeholder={placeholder||""} style={{border:`1px solid ${T.greyM}`,borderRadius:8,padding:"9px 14px",fontSize:13,fontFamily:"'Source Sans 3',sans-serif",background:readOnly?T.off:T.white,color:T.navy}}/>}
  </div>
);
  ...
  return(<div style={{minHeight:"100vh",background:`linear-gradient(135deg,${T.navy} 0%,${T.slate} 55%,${T.navyMid} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{background:T.white,borderRadius:20,padding:"44px 40px",width:400,boxShadow:"0 24px 64px rgba(0,0,0,0.35)"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <button onClick={go} style={{background:T.coral,color:T.white,border:"none",borderRadius:10,padding:"13px",fontSize:15,fontWeight:700,fontFamily:"'Source Sans 3',sans-serif",cursor:"pointer"}}>Sign In →</button>
      </div>
...
function Sidebar({user,page,setPage,onLogout}){
  ...
  return(<div style={{width:248,minHeight:"100vh",background:`linear-gradient(180deg,${T.navy} 0%,${T.slate} 100%)`,display:"flex",flexDirection:"column",flexShrink:0,position:"fixed",top:0,left:0,bottom:0,zIndex:100,overflowY:"auto"}}>
    ...
    <div style={{padding:"14px 16px",borderTop:"1px solid rgba(255,255,255,0.1)"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:"50%",background:T.coral,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:13,color:T.white,flexShrink:0}}>{user.avatar}</div><div style={{flex:1,minWidth:0}}><div style={{color:T.white,fontSize:12,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.name}</div><div style={{fontSize:10,color:"rgba(255,107,107,0.9)",background:"rgba(255,107,107,0.18)",padding:"2px 8px",borderRadius:20,display:"inline-block",marginTop:2}}>{user.role}</div></div><span onClick={onLogout} style={{color:"rgba(255,255,255,0.35)",cursor:"pointer",fontSize:18}}>⇠</span></div></div>
  </div>);
}
    ...
    <div style={{padding:"24px 32px"}}>
      ...
      <div style={{background:T.white,borderRadius:12,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.06)",border:`1px solid ${T.greyM}`}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>
            {vis.map((b,i)=>{const c=comp(b.component_id);return(<tr key={b.id} className="row-hover" style={{background:i%2===0?T.white:T.off,borderBottom:`1px solid ${T.greyL}`,transition:"background 0.15s"}}>
              <td style={{padding:"13px 16px"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:38,height:38,borderRadius:"50%",background:aColor(b.name),display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:13,color:T.white,flexShrink:0}}>{inits(b.name)}</div><div><div style={{fontWeight:700,fontSize:13,color:T.navy}}>{b.name}</div><div style={{fontSize:11,color:T.grey}}>Age {b.age} · {b.gender} · {b.bid}</div></div></div></td>
        ...
        <div>
          ...
          <div style={{background:T.white,borderRadius:12,padding:"24px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
            ...
            {tab==="followups"&&<div>
              ...
              {(ben.posts||[]).slice().reverse().map(p=>(<div key={p.id} style={{background:T.off,borderRadius:10,padding:"14px 16px",marginBottom:12,borderLeft:`3px solid ${T.coral}`}}><div style={{fontSize:11,color:T.grey,marginBottom:6}}>📅 {p.date} · ✍️ {p.author}</div><div style={{fontSize:13,color:T.navy,lineHeight:1.6}}>{p.text}</div></div>))}
            </div>}
    ...
    <div style={{padding:"24px 32px"}}>
      ...
      <div style={{background:T.white,borderRadius:12,padding:"36px 40px",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{marginBottom:24}}>
          ...
          {(!ben.posts||ben.posts.length===0)?<div style={{color:T.grey,fontSize:13}}>No follow-up notes recorded.</div>:(ben.posts||[]).map((p,i)=>(<div key={i} style={{marginBottom:12,padding:"10px 14px",background:T.off,borderRadius:8,borderLeft:`3px solid ${T.coral}`}}><div style={{fontSize:11,color:T.grey,marginBottom:4}}>Date: {p.date} | Officer: {p.author}</div><div style={{fontSize:13,color:T.navy,lineHeight:1.6}}>{p.text}</div></div>))}
        </div>
